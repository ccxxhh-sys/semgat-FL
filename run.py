...
def _get_opt(flag, env_key, default=""):
    if flag in sys.argv:
...
def _get_bool_opt(flag, env_key, default=False):
    val = _get_opt(flag, env_key, "1" if default else "0")
    return str(val).lower() in ("1", "true", "yes", "y")

def _get_float_opt(flag, env_key, default=0.0):
    val = _get_opt(flag, env_key, str(default))
    try:
        return float(val)
    except Exception:
        return default

def _get_int_opt(flag, env_key, default=0):
    val = _get_opt(flag, env_key, str(default))
    try:
        return int(val)
    except Exception:
...
args = dotdict({
    ...
    'seed':0,
    'lr':1e-3,
    'UseCompact': _get_bool_opt("--use-compact", "USE_COMPACT", False),
    'CompactMapPath': _get_opt("--compact-map", "COMPACT_MAP", ""),
    'CompactEmbPath': _get_opt("--compact-emb", "COMPACT_EMB", ""),
    'EdgeGateL1': _get_float_opt("--edge-gate-l1", "EDGE_GATE_L1", 1e-5),
    'EdgeGateWarmup': _get_int_opt("--edge-gate-warmup", "EDGE_GATE_WARMUP", 3),
    'UseGCNNBaseline': _get_bool_opt("--use-gcnn-baseline", "USE_GCNN_BASELINE", False),
})
...
def train(t = 5, p='Math'):
    ...
    for epoch in range(15):
        if args.EdgeGateL1 and args.EdgeGateL1 > 0:
            if args.EdgeGateWarmup and args.EdgeGateWarmup > 0:
                scale = min(1.0, max(0.0, float(epoch) / float(args.EdgeGateWarmup)))
                model.edge_gate_l1 = args.EdgeGateL1 * scale
            else:
                model.edge_gate_l1 = args.EdgeGateL1
        else:
            model.edge_gate_l1 = 0.0
        print(f"[gate] epoch={epoch} edge_gate_l1={model.edge_gate_l1:.2e}")
        index = 0
        for dBatch in tqdm(train_set.Get_Train(args.batch_size)):
            if index == 0:
                ...
                eval_bs = len(val_set)
                if p == "Time":
                    eval_bs = min(len(val_set), 4)
                for k, devBatch in tqdm(enumerate(val_set.Get_Train(eval_bs))):
            ...
            if index == 0:
                print(f"[gate] mean_gate={getattr(model, 'last_gate_l1', 0.0):.4f}")
