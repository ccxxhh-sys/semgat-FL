...
class NlEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.transformerBlocks = nn.ModuleList(
            [TransformerBlock(self.embedding_size, 8, self.feed_forward_hidden, 0.1, use_gcnn=getattr(args, "UseGCNNBaseline", False)) for _ in range(5)])
        ...
        self.transformerBlocksTree = nn.ModuleList(
            [rightTransformerBlock(self.embedding_size, 1, self.feed_forward_hidden, 0.1) for _ in range(5)])
        ...
        self.edge_gate_l1 = getattr(args, "EdgeGateL1", 0.0)
        self.last_gate_l1 = 0.0
        self.use_gcnn = getattr(args, "UseGCNNBaseline", False)
    def forward(self, input_node, inputtype, inputad, res, inputtext, linenode, linetype, linemus, modification, churn):
        ...
        node_type = torch.cat([inputtype, linetype], dim=1).long()
        gate_l1_total = x.new_zeros(x.size(0))
        for trans in self.transformerBlocks:
            x, gate_l1 = trans.forward(x, nlmask, inputad, node_type)
            gate_l1_total = gate_l1_total + gate_l1
        ...
        loss = loss.sum(dim=-1)
        if (not self.use_gcnn) and self.edge_gate_l1 and self.edge_gate_l1 > 0:
            loss = loss + self.edge_gate_l1 * gate_l1_total
        self.last_gate_l1 = float(gate_l1_total.detach().mean().item())
